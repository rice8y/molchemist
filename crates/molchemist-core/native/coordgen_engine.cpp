...
bool molchemist::layout_coordinates(
    const uint8_t* input_data,
    size_t input_size,
    std::vector<uint8_t>& output,
    std::string& error)
{
    const std::vector<uint8_t> input(input_data, input_data + input_size);
    if (input.size() < 20 || std::memcmp(input.data(), kLayoutMagic, 4) != 0)
    {
        return fail(error, "Invalid layout payload");
    }
    ...
    {
        return fail(error, "Invalid layout payload header");
    }
    ...
    {
        return fail(error, "Layout payload length does not match atom/bond counts");
    }
    ...
    for (uint32_t i = 0; i < bond_count; ++i)
    {
        ...
        {
            delete molecule;
            return fail(error, "Bond payload ended unexpectedly");
        }
    ...
    for (uint32_t i = 0; i < atom_stereo_count; ++i)
    {
        ...
        {
            delete molecule;
            return fail(error, "Atom stereo payload ended unexpectedly");
        }
    ...
    for (uint32_t i = 0; i < double_bond_stereo_count; ++i)
    {
        ...
        {
            delete molecule;
            return fail(error, "Double-bond stereo payload ended unexpectedly");
        }
    ...
    for (const auto& record : atom_stereo)
    {
        ...
        if (record.atom >= atom_count || bad_neighbor_index ||
            (record.direction != 1 && record.direction != 2))
        {
            delete molecule;
            return fail(error, "Atom stereo payload contains an invalid atom index");
        }
    ...
    for (const auto& record : double_bond_stereo)
    {
        ...
        {
            delete molecule;
            return fail(error, "Double-bond stereo payload contains an invalid index");
        }
